...
class KenKenGUI:
    ...
    def draw_board(self):
        ...
        for r in range(size):
            for c in range(size):
                ...
                fill_color = COLORS['selected'] if (r, c) == self.selected_cell else 'white'
                self.canvas.create_rectangle(x1, y1, x2, y2, fill=fill_color, outline=COLORS['grid_line'])

        # 2. Draw Cages (Thick Borders)
        ...
        for r in range(size):
            for c in range(size):
                ...
                # Check neighbors to draw thick lines
                if r==0 or cage_map.get((r-1,c)) != cid: # Top
                    self.canvas.create_line(x1, y1, x2, y1, width=3, fill=COLORS['cage_border'])
                if r==size-1 or cage_map.get((r+1,c)) != cid: # Bottom
                    self.canvas.create_line(x1, y2, x2, y2, width=3, fill=COLORS['cage_border'])
                if c==0 or cage_map.get((r,c-1)) != cid: # Left
                    self.canvas.create_line(x1, y1, x1, y2, width=3, fill=COLORS['cage_border'])
                if c==size-1 or cage_map.get((r,c+1)) != cid: # Right
                    self.canvas.create_line(x2, y1, x2, y2, width=3, fill=COLORS['cage_border'])

        # 3. Draw Targets
        for cage in self.current_puzzle.cages:
            cells = cage['cells']
            top_left = min(cells, key=lambda x: (x[0], x[1]))
            tx = start_x + top_left[1] * self.cell_size + 5
            ty = start_y + top_left[0] * self.cell_size + 5
            label = f"{cage['target']}{cage['operation']}"
            self.canvas.create_text(tx, ty, text=label, anchor="nw", font=("Segoe UI", 9, "bold"), fill=COLORS['text'])
        ...
        for r in range(size):
            for c in range(size):
                ...
                if val != 0:
                    ...
                    self.canvas.create_text(cx, cy, text=str(val), font=("Segoe UI", 18), fill=COLORS['text'])
    ...
    def _build_cage_map(self):
        m = {}
        for i, cage in enumerate(self.current_puzzle.cages):
            for cell in cage['cells']: m[cell] = i
        return m
    ...
    def on_canvas_click(self, event):
        ...
        c = int((event.x - ox) // self.cell_size)
        r = int((event.y - oy) // self.cell_size)
    ...
    def toggle_ui(self, enable):
        ...
        self.btn_ca.config(state=state)
    ...
    def on_solve_done(self, res, name):
        ...
        if name == "Cultural Algo" and 'fitness_history' in res:
            self.show_plot(PerformanceAnalyzer.plot_fitness_progression(res), "Fitness Graph")

    def on_compare_done(self, res):
        self.toggle_ui(True)
        self.status_var.set("Comparison Complete.")
        bt = res['backtracking']
        ca = res['cultural_algorithm']
